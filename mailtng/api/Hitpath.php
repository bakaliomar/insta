<?php namespace ma\mailtng\api
{
    ...
    class Hitpath extends Base
    {
                ...
                foreach ($response['data'] as $offerData) 
                {
                    if(is_array($offerData) && count($offerData))
                    {
                        ...
                        if(in_array(intval($campaignId),$offerIds))
                        {
                            ...
                            $offer['rules'] = '';
							 $links = $this->getResponse(array('apifunc' => 'getsuppression','campaignid' => $campaignId));
                            if(count($links['data']))
                            {
                            	  $opolo = $links['data'];
								  $offer['suppression-list-link'] = explode(PHP_EOL,$opolo['suppurl']);
	   
                                }else{
	   
	                           $offer['suppression-list-link'] ='no_sup_list' ;
	   
                                  }
                            $offer['epc'] = trim(preg_replace('/\s\s+/','',$offerData['unit']));
                            $offer['flag'] = 'US';
                            $offer['key'] = 'c2';
                ...
                foreach ($response['data'] as $offerData) 
                {
                    if(is_array($offerData) && count($offerData))
                    {
                        ...
                        if($offerId != 0 && intval($campaignId) == intval($offerId))
                        {
                            ...
                            {
								$opoloo = $links['data'];
								  $offer['suppression-list-link'] = explode(PHP_EOL,$opoloo['suppurl']);
                                }else{
        ...
        public function getMonthEarning()
        {
	       	   ...
	       	   $d=strtotime("today");
               $m=strtotime("first day of this month");
               $earnings = 0.0;
	       		
	           $service_url = $this->_url;
               $report =    str_replace('pubapi.php','api.php',$service_url);
               $report =   str_replace('http://api', 'http://reporting', $report);	
               $curl = curl_init($report);
               $curl_post_data = array("key" => $this->getKey(),"type" => 'sales',"start" => date("m/d/Y ", $m),"end" => date("m/d/Y ", $d),"format" => 'xml',);
