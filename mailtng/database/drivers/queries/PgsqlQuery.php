<?php namespace ma\mailtng\database\drivers\queries
{
    ...
    /**
     * @name            PgsqlQuery.class 
     * @description     The postgres queries class
     * @package		ma\mailtng\database\drivers\queries
     * @category        Database Class
     * @author		MailTng Team			
     */
    class PgsqlQuery extends Base
    {
        /** 
         * @readwrite
         * @access protected 
         ...
         * @throws DatabaseException
         */
        public function save($data,$schema = null,$returnId = true) 
        {  
            $sequenceName = null;
            ...
            if ($isInsert)
            {
                ...
                $fromParts = explode('.',$this->_from);
                $sequenceName = $schema ."seq_id_" . $fromParts[1];
            }
            else
            {
            ...
            $this->_connector->executeQuery($sql,false,\PDO::FETCH_ASSOC,true,$sequenceName);
         ...
         * @throws DatabaseException
         */
        public function createTable($model,$tableName = null,$schema = null) 
        {
            if(isset($model))
            {
                $lines = array();
                ...
                $schema = $schema != null ? $schema . "." :  $model->getSchema() . ".";

                # make sure that the schema is not only "."
                $schema = trim($schema) == "." ? null : $schema;
                ...
                foreach ($columns as $column) 
                {
                    ...
                    if ($column["primary"]) 
                    {
                        $indices[] = "CONSTRAINT c_pk_{$name}_{$table} PRIMARY KEY({$name}) ";
                    }

                    if ($column["autoincrement"]) 
                    {
                        $sequenceName = $schema . "seq_{$name}_{$table}";
                        $sequenceQuery = "CREATE SEQUENCE $sequenceName START 1;";
                    }
                ...
                $tableQuery = sprintf($template,$schema . $table, join(",\n", $lines), join(",\n", $indices), $this->_connector->getEngine(), $this->_connector->getCharset());

                # sequence section
                if($sequenceName != '')
                {
                    # droping the sequence if exists
                    $this->_connector->executeQuery("DROP SEQUENCE IF EXISTS {$sequenceName};");

                    # creating a new sequence 
                    $result = $this->_connector->executeQuery($sequenceQuery);
                }
        ...
        protected function _quote($value) 
        {
            if (is_string($value))  
            {
                $escaped = strstr($value,"no_quote :") ? trim(str_replace("no_quote :","", $value)) : $this->_connector->escape($value);
                return "{$escaped}";
            }
            if (is_array($value)) 
            {
